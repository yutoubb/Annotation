...
my (%hash_line,$chr,@AD,$ratio,$ratio1,$ratio2,$Mtype,$gene,$fuction,$exon,$exon_all,$trans,$cHGVS,$pHGVS,@b,$rs_id,@c,$end,$c_len,@variation,$line,$protein_num);
...
#			($cHGVS) = $a[37] =~ m/^[A-Z0-9\._]+:(c\.\w+)/;
			...
			unless($pHGVS){
				if($a[6] =~ m/^synonymous/){
					my ($site_for_p) = $cHGVS =~ /c.(\d+)[A-Z]/;
					my $remainder = $site_for_p % 3;
					if($remainder eq '0'){
						$protein_num = $site_for_p / 3;
					}
					else{
						$protein_num = int($site_for_p/3) + 1;
					}
					my ($condon_ref,$condon_alt) = $a[11] =~ /(\w+)\/(\w+)/;
					$pHGVS = 'p.'.$condon_ref.$protein_num.$condon_alt;
				}
				else{
					$pHGVS = '-';
				}
